...
``` 
```{r include=FALSE}
#f1v2 <- as.data.frame(L1v2_res)
#  df2v3 <- as.data.frame(L2v3_res)
#  df1v3 <- as.data.frame(L1v3_res)
  

# df1v2$gene_id <- rownames(df1v2)
#   df2v3$gene_id <- rownames(df2v3)
#   df1v3$gene_id <- rownames(df1v3)
#   L1v2_ds_nor_counts$gene_id <- rownames(L1v2_ds_nor_counts)
#   
# ref_table <- read.csv("/mnt/nas2/elis/ref/chm13.draft_v2.0.gene_annotation.sorted.filtered.vfilter.csv")
# 
# df1v2_table <- merge(ref_table, df1v2, by = "gene_id")
#   loss_gene <- df1v2[! (rownames(df1v2)%in%ref_table$gene_id),]
# 
# df2v3_table <- merge(ref_table, df2v3, by = "gene_id")
#   loss_gene <- df2v3[! (rownames(df2v3)%in%ref_table$gene_id),]
# 
# df1v3_table <- merge(ref_table, df1v3, by = "gene_id")
#   loss_gene <- df1v3[! (rownames(df1v3)%in%ref_table$gene_id),]
#   
# L1v2_ds_nor_counts_table <- merge(ref_table, L1v2_ds_nor_counts, by = "gene_id")
#   L1v2_ds_nor_counts_table <- merge(L1v2_ds_nor_counts_table, df1v2, by = "gene_id")
#   L1v2_ds_nor_counts_table <- merge(L1v2_ds_nor_counts_table, df2v3, by = "gene_id")
#   L1v2_ds_nor_counts_table <- merge(L1v2_ds_nor_counts_table, df1v3, by = "gene_id")
#   write.csv(L1v2_ds_nor_counts_table,"/mnt/nas2/yh/10.RNA_seq_poly_A/8.count_ucsc/L1v2_ds_nor_counts_table20240206.xlsx")
# data <- df1v3_table
####
```
